...
// Setup RecyclerView
        ...
        adapter = ChatAdapter(
            ...
            onMessageLongClick = { chat ->
                showMessageOptionsDialog(chat)
            }
        )
...
// Start TaskInfoActivity
            ...
            finish() // This will close the chat activity
        }
    }
                ...
                db.document("groups/$groupId")
                    ...
                    .addOnSuccessListener { groupDoc ->
                        ...
                        if (!members.contains(currentUserId)) {
                            Toast.makeText(this, "No access to this chat", Toast.LENGTH_SHORT)
                                .show()
                            finish()
                            return@addOnSuccessListener
    ...
    private fun sendMessage(type: String = "text", attachmentUrl: String = "") {
        ...
        binding.sendButton.isEnabled = false
// Correct path to chats collection (inside task)
        ...
        db.collection("groups").document(groupId).collection("tasks").document(taskId).collection("chats")
            ...
            .addOnFailureListener { e ->
                Toast.makeText(this, "Failed to send message: ${e.message}", Toast.LENGTH_LONG)
                    .show() // Show error message

                Log.e("ChatActivity", "Error sending message", e) // Log the full error
                binding.sendButton.isEnabled = true
            }
    }
    ...
    private fun deleteMessage(chat: Chat) {
        ...
        if (chat.senderId != currentUser.uid) {
            ...
            Toast.makeText(this, "You cannot delete other users' messages", Toast.LENGTH_SHORT)
                .show()
            return

        }
// Correct path to the specific message document (inside task)
        db.collection("groups").document(groupId).collection("tasks").document(taskId).collection("chats")
            .document(chat.messageId).update(mapOf("content" to "", "isDeleted" to true))
            .addOnSuccessListener {
                Toast.makeText(this, "Message deleted", Toast.LENGTH_SHORT).show()
            }
            .addOnFailureListener { e ->
                Toast.makeText(this, "Failed to delete message: ${e.message}", Toast.LENGTH_LONG)
                    .show() // Show error message
                Log.e("ChatActivity", "Error deleting message", e) // Log the full error
        ...
        when (requestCode) {
            ...
            STORAGE_PERMISSION_CODE -> {
                if (grantResults.isNotEmpty() &&
                    ...
                    Toast.makeText(
                        this, "Permission denied, cannot download files",
                        Toast.LENGTH_SHORT
                    ).show()
